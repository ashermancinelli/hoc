...
#xref(<fig:dawn-timeline>, capital: false), and the pace of development in Hopper's time compared
to the years prior.Note that while we have ample data on #emph[how] Hopper's
...
it was significantly slower than the ENIAC, it was #emph[programmable]; the
...
the #emph[Manual of Operation for the Automatic Sequence Controlled
...
#cite(<grace_hopper_and_the_invention_of_the_information_age_2009>, form: "normal", supplement: [interview on 5 July, 1972])
...
manual.#cite(<annals_of_the_computation_laboratory_of_harvard_university_1946>, form: "normal"):
...
=== Hopper and the Mark I's Manual
...
#list(
  [Blaise Pascal's counting machines, "foundation on which
	      nearly all mechanical
	      calculating machines since have been constructed."],
  [Leibniz; stepped wheels system for mul/divs.],
  [Charles Babbage; most significant part of the manual dedicated to him.
	      ...
	      von neumann).],
  [Ada King, Countess of Lovelace; series of essays on Babbage's machine.
...
#emph[Symposium on Large Scale Digital Computing Machinery]in 1947 to foster
...
In 1949 when people knew I had run out the time at
	...
	#cite(<Hopper_1980_Oral_History>, form: "normal")
]
...
#cite(<grace_hopper_and_the_invention_of_the_information_age_2009>, form: "normal") creating a
...
Another Hopper programmer, Adele Mildred Koss, was assigned to Commonwealth
	...
	#cite(<grace_hopper_and_the_invention_of_the_information_age_2009>, form: "normal", supplement: [Adele Mildred Koss, interviewed by Kathy
		Kleiman])
]
...
Having built his career on punch cards, Dad distrusted magnetic
	...
	reused.#cite(<grace_hopper_and_the_invention_of_the_information_age_2009>, form: "normal")
]
...
Inspired by Holberton's Sort-Merge Generator, Hopper conceived the
	...
	#cite(<women_in_computing_history_2002>, form: "normal")
]
...
At this time we should note that the term #emph[compiler] had not yet taken
on its modern meaning. Hopper used the terms #emph[automatic programming] and
#emph[compiler] to refer to programs that produce other programs, but they
...
present day understanding. Let us turn to the 1952 paper #emph[The Education
	of a Computer] in which Hopper announced her A-0 compiler, which she presented
at a Pittsburgh ACM meeting#cite(<education_of_a_computer_1952_hopper>, form: "normal").

#figure(
  [#image("../../resource/gh_education_of_a_computer_type_fig_6.png", width: 70%)],
  kind: image,
  caption: [Depiction of #emph[type-B compilation] from Hopper's
		#emph[The Education of a Computer]]
) <fig:education-of-a-computer-1952-hopper-f6>
...
locations into a single output program based on an input program #emph[that
...
#cite(<education_of_a_computer_1952_hopper>, form: "normal").
One may also consider this effort
to be the first #emph[standard library], which would become a major feature
...
#cite(<ridgway_compiling_routines_1952>, form: "normal"). For this study, Richard compared the
computer and labor time spent to calculate a table of values for the equation:


$ y = e^(-x^2) sin((x) / (2))
	quad upright("for ") |x| < 1, Delta x = 0.01. $
...
Richard writes in #emph[Compiling Routines]#cite(<ridgway_compiling_routines_1952>, form: "normal"):
...
in the form of #emph[pseudocode], which was a (slightly) more human-friendly
format than plain machine code.
#todo[Some sources reference the A-2 compiler instruction manual,
...
#cite(<pentagon_hopper_univac_workshop_1953>, form: "normal"),
...
=== After the A-2: A-3, AT-3, MATH-MATIC


#todo[Should make some mention of interpreters in here.
...
This debate is covered in Section #xref(<sec:backus_priesthood>, capital: false);
...
#todo[Adele Mildred Koss, interview by Kathy Kleiman, 1993,
...
components of the MATH-MATIC. The AT-3 was the #emph[Translator]
and the A-3 was the #emph[Arith-Matic Compiler]
#cite(<ash_etal_1957_math-matic_manual>, form: "normal").

#figure(
  [#image("../../resource/mathmatic-user-guide.png", width: 70%)],
  kind: image,
  caption: [Excerpt from the MATH-MATIC User's Manual, 1957]
) <fig:mathmatic-user-manual>


It is worth noting that #cite-title(<grace_hopper_and_the_invention_of_the_information_age_2009>)
#cite(<grace_hopper_and_the_invention_of_the_information_age_2009>, form: "normal")
...
This led me to believe that the A-3 #emph[became] the AT-3 which was
renamed to the MATH-MATIC; however, the MATH-MATIC user manual
#cite(<ash_etal_1957_math-matic_manual>, form: "normal") specifies that the MATH-MATIC
...
respectively).
See Figure #xref(<fig:mathmatic-user-manual>, capital: false).
...
#cite(<Knuth_TrabbPardo_1976_Early_Development>, form: "normal")
Perhaps the #emph[language] A-3 was extended to become AT-3 which was
...
But the MATH-MATIC programmers did not share the FORTRAN group's enthusiasm for
	...
	Knuth and Pardo#cite(<Knuth_TrabbPardo_1976_Early_Development>, form: "normal").
]


=== The B-0, FLOW-MATIC, and COBOL
...
#quote(block: true)[
#strong[Pantages:] At that point did you have a feeling for what was
	happening, in terms of what you were contributing?

	#strong[Hopper:] No. I've always objected to doing anything over again if I had
...
#figure(
  [#image("../../resource/gh_data_processing_compiler.png", width: 70%)],
  kind: image,
  caption: [Depiction of Hopper's Data Processing Compiler#cite(<hopper_1955_preliminary_definitions_data_processing_compiler>, form: "normal")]
) <fig:data-processing-compiler>
...
#cite-title(<hopper_1955_preliminary_definitions_data_processing_compiler>)
#cite(<hopper_1955_preliminary_definitions_data_processing_compiler>, form: "normal")
.
...
This was what she originally called the data processing compiler in January,
	...
	#cite(<history_of_computing_in_the_twentieth_century_1980>, form: "normal")
]


#figure(
  [#image("../../resource/flow-matic-example-knuth-pardo.png", width: 70%)],
  kind: image,
  caption: [Example FLOW-MATIC code from Knuth and Trabb Pardo#cite(<Knuth_TrabbPardo_1976_Early_Development>, form: "normal")]
) <fig:knuth-pardo-flow-matic-example>
...
#cite(<Knuth_TrabbPardo_1976_Early_Development>, form: "normal")
(see also Figure #xref(<fig:knuth-pardo-flow-matic-example>, capital: false)).
...
her team to UNIVAC customers to learn about their #emph[business] needs.

#figure(
  [#image("../../resource/flow-matic-ad-1959.png", width: 70%)],
  kind: image,
  caption: [Advertisement for FLOW-MATIC and UNIVAC II]
) <fig:flow-matic-ad-1959>
...
statements such as:
#mono-text("IF EQUAL GO TO OPERATION 5 ; OTHERWISE GO TO OPERATION 2 .")
...
in early 1958 in #cite-title(<sperryrand_1959_flowmatic>)#cite(<sperryrand_1959_flowmatic>, form: "normal").
In this manual, one now finds advertisements for the UNIVAC II#xref(<fig:flow-matic-ad-1959>, capital: false).
...
For example, the #mono-text("CLOSE-OUT") command is specified in the FLOW-MATIC manual as
...
$ Delta upright("op(\"CLOSE-OUT\")") Delta
	[
		mat(upright("file"); upright("files"))
		]
	Delta
	f_1
	Delta
...
#todo[In some places, Hawes is described as one of the B-0 developers which leads
...
Sammet states#cite(<sammet_early_history_of_cobol_1978>, form: "normal") that Block's 1959
#emph[Report on Meeting held at University of Pennsylvania Computing Center] names
Hawes as having called this meeting:

#quote(block: true)[
… meeting was the result of a request by
...
#todo[there is not much about Mary Hawes available online,
	I can't even find the papers Hawes originally wrote.
	Try to find #emph[Mary Hawes, “Automatic Routines for Commercial Installations”]]
...
#cite(<sammet_programming_languages_history_and_fundamentals_1969>, form: "normal", supplement: [Phillips, as quoted by Sammet in]).
...
#emph[Three separate committees] were formed for this purpose; a short, medium, and long-term committee.
...
development of COBOL"#cite(<sammet_early_history_of_cobol_1978>, form: "normal").
She went on to say:
...
This wording is clearly ambiguous, and at the time, there was some discussion
	...
	clear #emph[then] that we were to do anything #emph[except] try and
	#emph[combine] the three known languages of the time.
	These were FLOW-MATIC, … AIMACO, … and COMTRAN
]
...
was almost entirely FLOW-MATIC#cite(<Hopper_1980_Oral_History>, form: "normal"), though the committee notes
...
Most of the disdain for COBOL may indeed come from the fact that it was designed by the #emph[short term]
...
=== COBOL Comes Into Focus

 <subsec:cobol-comes-into-focus>
...
$ mat(upright("BUSY"), upright("Business System"); upright("BUSYL"), upright("Business System Language"); upright("INFOSYL"), upright("Information System Language"); upright("DATASYL"), upright("Data System Language"); upright("COSYL"), upright("Common Systems Language"); upright("COCOSYL"), upright("Common Computer Systems Language")) $


On September 18th of that year, the committee settled on
COBOL, according to Jean Sammet's personal notes#cite(<sammet_early_history_of_cobol_1978>, form: "normal").
...
#emph[Basic COBOL], and that no computer manufacturer could claim to support COBOL
