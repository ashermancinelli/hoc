...
In terms of programming language design, Lisp's fundamental data structure is the #emph[list]--
hence the name (#emph[#strong[LIS]t #strong[P]rocessing]).
Symbolic computing (which was still novel at the time) was implemented using lists and basic
operations on them.
Lisp #emph[programs] were also represented as data; one of the key innovations of the language
the ability manipulate Lisp programs as any other data structure, with the #mono-text("eval")
...
==== FORTRAN Lisp Processing Language
...
#cite(<mccarthy_history_of_lisp_1978>, form: "normal").
Around this time, McCarthy was presented the list-processing programming language #emph[IPL 2]
(#emph[Information Processing Language]), written for the RAND Corporation's JOHNNIAC computer.
...
#cite-title(<mccarthy_programs_with_common_sense_1958>).
This paper, also called the #emph[Advice Taker] proposal,
...
#footnote[This is the layout as McCarthy describes in #cite-title(<mccarthy_history_of_lisp_1978>),
	but #cite(<gelernter_flpl_1960>, form: "author") include a sign bit as the uppermost bit of the tag.]:

#figure(
  [#table(columns: 5, inset: 5pt, stroke: 0.4pt,
  [],
  [#strong[Tag]],
  [#strong[Decrement]],
  [#strong[Prefix]],
...
in FORTRAN, called FLPL for #emph[FORTRAN-Compiled List Processing Language].
...
They note in #cite-title(<gelernter_flpl_1960>) that while IPL's interpretation was prohibatively
...
performance-critical sections of their list-processing programming environment#cite(<gelernter_flpl_1960>, form: "normal"):
...
#cite(<gelernter_flpl_1960>, form: "author") conclude their paper by pointing this out:
...
of the conditional expression were always evaluated; #mono-text("XIF(C, E1, E2)") would return
#mono-text("E1") if #mono-text("C") was equal to 1 and #mono-text("E2") otherwise, which we would now call
a #emph[merge] or #emph[select] operation.
This was motivated by the clumsy syntax and semantics of the #mono-text("IF") statement in FORTRAN I and II,
...
he added #mono-text("maplist") to apply a function to each element of a list,
the first his many steps towards higher-order programming.
To pass functions as arguments to #mono-text("maplist") and other higher-order functions,
he would need a notation for functions, which he introduced with #mono-text("lambda")
#cite(<mccarthy_history_of_lisp_1978>, form: "normal"):

#quote(block: true)[
use functions as arguments, one needs a notation for functions, and it seemed
	natural to use the λ-notation of Church (1941).
...
to develop; John Backus's team noted in #cite-title(<backus_etal_fortran_automatic_coding_system_1957>)
...
They instead began developing a Lisp #emph[environment], at this point a set of
functions compiled by hand for reading and printing list structures.
These programs were written in #emph[M-expressions], an informal language resembling
FORTRAN with assignments, #mono-text("goto")s, the core Lisp functions, and a few
of the features they had wanted from FORTRAN but couldn't get, namely recursive functions.

Beginning with #cite-title(<mccarthy_recursive_functions_computation_1960>), McCarthy and the
team at MIT began writing papers describing theories in Lisp.
The #cite(<mccarthy_recursive_functions_computation_1960>, form: "year") paper was for using Lisp
...
turing machines by formally describing a #emph[universal Lisp function],
...
The number 0 representing both the empty list #mono-text("NIL") and the boolean value
#mono-text("FALSE") is one such case.

The prefix-notation and use of parentheses (also known as #emph[S-expressions])
...
#mono-text("quote"), #mono-text("atom"), #mono-text("eq"), #mono-text("car"), #mono-text("cdr"), #mono-text("cons"), and #mono-text("cond"),
they could implement a #emph[total language], or a language capable of expressing
any Lisp function.
...
We will not cover them exhaustively here, but McCarthy's reccolection in #cite(<mccarthy_history_of_lisp_1978>, form: "normal")
seems to be the authoratative source and the list is short.
The one we will cover is #emph[lexical scoping].
...
to find a subexpression in #mono-text("x") such that the predicate #mono-text("p(x)") is true,
and to return #mono-text("f(x)") if so.
If the search did not find such an #mono-text("x"), then the function containing
...
#emph[where it was called] and not the scope #emph[where it was defined].
...
Today, these two semantics for variable scoping are called #emph[lexical scoping]
...
#emph[where the variable is used]. In this case, #mono-text("(car x)") is looked up in the
...
#cite(<revised_report_on_the_algorithmic_language_algol_68_1976>, form: "normal", supplement: [Section 4.7.3.3. Body replacement and execution]).
...
Russell eventually did implement a solution for James through the #mono-text("FUNARG")
...
#cite(<weizenbaum_funarg_explained_1968>, form: "normal")
and
#cite(<moses_function_of_function_1970>, form: "normal")
...
The "functions" #mono-text("FEXPRS") and #mono-text("FSUBRS") are not given regular arguments, but are given expressions
that will evaluate to their arguments.
The function bodies can use Lisp's #mono-text("eval") function to actually evaluate their arguments in the body of the
...
#mono-text("FEXPRS") and #mono-text("FSUBRS"), or they could evaluate some of them at the start
and some of them later, and so on.


==== The First Lisp Compilers


#cite(<allen_anatomy_of_lisp_1978>, form: "author") collected the early histories of Lisp compilers
in #cite(<allen_anatomy_of_lisp_1978>, form: "normal")
from a few primary sources that are difficult to find, namely
the paper #cite-title(<blair_structure_lisp_compiler_1971>) by #cite(<blair_structure_lisp_compiler_1971>, form: "author"),
...
The first was Robert Brayton at MIT #cite(<blair_structure_lisp_compiler_1971>, form: "normal"),
...
As #cite(<blair_structure_lisp_compiler_1971>, form: "author") recalls in #cite-title(<blair_structure_lisp_compiler_1971>),
...
evaluation semantics, and prefers the model he describes in #cite-title(<landin_eval_of_expressions_1964>)
...
the time (#emph[numbers] were only just added in the first version), the I/O
...
instruction set was defined with Lisp in mind#cite(<mccarthy_history_of_lisp_1978>, form: "normal").
...
#todo[Continue here: #cite-title(<steele_evolution_lisp_96>).
	First cross-compiler to bring up a language.
	Just had to port/cross compile the "subconscious" #cite(<allen_anatomy_of_lisp_1978>, form: "normal", supplement: [pp 289]).
	Teams were generally isolated and had a few people that knew lisp really well
	and everyone else just used the language.
	Maclisp and Interlisp compilers.
	Richard Stallman worked on MIT Lisp Machine project in 1974.
	The DWIM/Do what I mean lisp that would spell-correct or fix syntax
	errors and go ahead and do what the user intended(!!), and
	had an ast-based editor(!!).
	NLAMBDA vs LAMBDA, more calling conventions.
	PSL, portable standard lisp. Mostly in the 70s and 80s now.
	"The Portable Lisp Compiler (PLC) compiled Lisp
	code into an abstract assembly language. This language was then converted to a machine-dependent
	LAP (Lisp Assembly Program) format by pattern-matching" wow!
]

#todo[mention #gls("lisp-machine")s so we can refer back to them in #xref(<sec:gnu>, capital: false)].
#todo[mention #cite(<kessler_peep_optimizations_1984>, form: "normal") compiler optimizations
	originally deployed to lisp compilers.]
