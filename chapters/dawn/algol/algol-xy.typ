...
#cite(<cleaning_up_algol60_duncan_wijngaarden_1964>, form: "normal").
...
formal grammar which came to be known as a #emph[W-Grammar] (after his own namesake).
...
(which was, mind you, supposed to be the #emph[incremental] improvement to ALGOL 60)
...
in six days"#cite(<a_history_of_algol_68_1993>, form: "normal").

Wijngaarden's ALGOL W proposal was distinct for at least three reasons:

#enum(
  [A two-level #emph[W Grammar].],
  ["The combination of a minimal number of language concepts in an orthogonal way"
	      #cite(<a_history_of_algol_68_1993>, form: "normal").],
  [Expression orientation; everything is an expression and has a value.],
)
...
#cite(<revised_report_on_the_algorithmic_language_algol_68_1976>, form: "normal")
(and numerous other texts on ALGOL 68) describes popular sentiment about the
report#cite(<a_history_of_algol_68_1993>, form: "normal"):
#quote(block: true)[
...
This was not his opinion, however#cite(<a_history_of_algol_68_1993>, form: "normal"):

#quote(block: true)[
I should point out that my own involvement with the project came after the basic design of the
	language, and of its original Report, were complete…
...
implementation of a subset of ALGOL 68, #emph[ALGOL 68S].
...
They even spent an afternoon on ALGOL Y, from which it emerged that no one had
	...
	programs and then #emph[eval] them.
]


The largest single outpouring of criticism of ALGOL 68 came from
#cite(<mr93_draft_report_on_algol_68_1968>, form: "normal"), also known as [MR93].
Several members of the committee (including Peter Naur) resigned
after this report's circulation.
It was first circulated to the #emph[ALGOL Bulletin] in February 1968,
and "was the cause of much shock, horror, and dissent, even
(perhaps especially) among the membership of WG 2.1"#cite(<a_history_of_algol_68_1993>, form: "normal").
Numerous papers were written in response to MR93, including Peter Naur's #emph[scathing]
critique #cite-title(<successes_and_failures_of_the_algol_effort_naur_1968>).
...
distributed his #cite(<algol68_with_fewer_tears_1968>, form: "normal")
in which he attempted to synthesize the nicer language hidden within the earlier reports
#footnote[For those unaware, this title is an homage to the English textbook #emph[French without Tears].].
...
For example, what ALGOL calls a #emph[name] is more commonly known as a reference or pointer;
numerous differences in nomenclature contributed to the difficulty of reading
the original reports.

Van Wijngaarden had circulated another
report#cite(<penultimate_draft_report_on_algol_68_1968>, form: "normal") in October of 1968 as he
...
meetings, the final report #cite-title(<report_on_the_algorithmic_language_algol_68_mailloux_1969>)
was published in #emph[Numerische Mathematik] and #emph[Kybernetika]
...
in order to be #emph[formally context-sensitive].
It is not merely a specification of the syntax of the language;
it also specifies a great deal of the #emph[semantics] of the language,
because that is required for the language to be context-sensitive.
When the grammar is described as #emph[two-level], it communicates that the
...
The #emph[format] of the grammar was also problematic and difficult to read,
...
#footnote[The term #emph[unification] may be loaded; Milner did not publish
...
grammar, where the #strong[#emph[MODE]] is #strong[#emph[int]], the #strong[#emph[destination]] is
the location of #strong[#emph[i]] of mode #strong[#emph[ref int]], and the source is
the integer literal #strong[#emph[5]] of #strong[#emph[MODE int]].
...
#emph[Section 4.4 Context Conditions],
which specified conditions for a program to be #emph[semantically valid]
based on the surrounding context.
Modern readers may find this strange; we are discussing the language #emph[syntax],
...
As we have already established, this is an example of language #emph[semantics]
being specified in the grammar.

Most modern compilers are broken into three parts: the front-end, middle-end
#footnote[Only compiler engineers could produce a term as idiosyncratic as #emph["middle" "end"].],
and back-end.
The front-end typically consumes the input program,
produces data structures representing the program,
and performs #emph[semantic] analysis on those data structures to ensure the program is valid.
...
See this excerpt on #strong[#emph[NEST]]s from
#cite(<revised_report_on_the_algorithmic_language_algol_68_1976>, form: "normal", supplement: [Section 3.0.2, Semantics]):
#quote(block: true)[
...
Today, #strong[#emph[NEST]]s might exist concretely as data structures inside the semantic analysis
...
#strong[#emph[union(int, real, bool)]],
#strong[#emph[union(real, int, bool)]], and
#strong[#emph[union(bool, union(real, int))]].
...
would be #emph[orthogonal]--#cite(<programming_language_pragmatics_2009>, form: "author")
put it well in #cite(<programming_language_pragmatics_2009>, form: "normal"):
...
One may argue that #emph[proceduring] is a form of call-by-name,
...
be assigned to) #emph[or] call-by-full-name (where the actual parameter
...
a #emph[new] parameter passing mechanism called #emph[parameterless #strong[procedure]]
parameters, meaning the parameters were thunks to be evaluated each time they
were used in the body of the callee#cite(<a_history_of_algol_68_1993>, form: "normal").
Lindsey uses an inner-product algorithm to illustrate this:

#code-file("chapters/dawn/inner-product.a68", lang: "algol")
...
Many of these coercion rules were uncontroversial; for example, a #strong[#emph[real]]
may be assigned to a #strong[#emph[union(real, int)]].
#todo[Discuss #emph[rowing]? relates to rank polymorphism a bit...]
One particularly problematic coercion rule was #emph[proceduring].
...
In #cite-title(<algol68_with_fewer_tears_1968>)
...
#cite(<revised_report_on_the_algorithmic_language_algol_68_1976>, form: "normal", supplement: [Section 11.2, Innerproduct 1]):

#code-file("chapters/dawn/inner-product2.a68", lang: "modula2")
...
=== ALGOL 68-R and the Revised Report


In 1973, the #cite-title(<revised_report_on_the_algorithmic_language_algol_68_1976>) was published,
codifying some of the changes and rejected features from the ALGOL 68-R team.

#todo[look at peck's #cite(<the_algol_68_story_peck_1978>, form: "normal")]
