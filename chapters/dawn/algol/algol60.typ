...
for formal specification of programming languages, #cite(<backus_ial_1959>, form: "normal").
...
in the paper before Naur's changes#cite(<backus_ial_1959>, form: "normal"):


$ ⟨ "factor" ⟩ :equiv
	⟨ "number" ⟩  overline(upright("or"))
	⟨ "function" ⟩  overline(upright("or"))
	⟨ "variable" ⟩  overline(upright("or"))
	dots.h $


Naur would make the notation more readable by replacing $overline(or)$ with $|$
...
titled #cite-title(<naur_backus_algol_1960>)#cite(<naur_backus_algol_1960>, form: "normal").
...
working on a compiler or interpreter was emblematic of the British
(and more broadly European) approach to programming language design.

// \parencite[6.2]{cardone_hindley_history_of_lambda_calcl_2006}


=== Adoption of ALGOL


The #emph[ACM Communications] started publishing an #emph[Algorithms] section;
ALGOL 58 was initially used, but ALGOL 60 was adopted once the updated report
was in wide circulation.
#mono[FORTRAN] was not considered acceptable for publication until 1966, and even then
...
In early 1960, the #emph[ACM ALGOL Maintenance Group] formed: an informal working
...
the #emph[ALGOL Bulletin]#cite(<sammet_programming_languages_history_and_fundamentals_1969>, form: "normal").
...
#footnote[The reader may find it interesting that in this edition of the bulletin,
	Jean Sammet submitted a paper on #emph[A Method of Combining ALGOL and COBOL];
...
#cite-title(<naur_revised_report_algol_1962>).


=== Peculiarities of ALGOL


The reader may find it interesting that ALGOL had a number of peculiar features.
Namely, the distinction between #emph[call by name] and #emph[call by value].
Naur's 1962 revised report explains it best, I think#cite(<naur_revised_report_algol_1962>, form: "normal", supplement: [Section 4.7.3]):

#quote(block: true)[
#strong[4.7.3.1 Value assignment (call by value)]

	of the corresponding actual parameters… the effect is as though an
	additional block embracing the procedure body were created in which these
	assignments were made to variables local to this fictitious block…


	#strong[4.7.3.2 Name replacement (call by name)]
...
#code-file("chapters/dawn/callbyname1.py", lang: "python")
...
#emph[#gls("l-value")s], or values in the left-hand side of an assignment expression.
...
which is the substituted into the body of the callee #emph[before]
being evaluated.

Donald Knuth proposed the #emph[man or boy] test in the #emph[ALGOL Bulletin]
to distinguish between
conformant ALGOL compilers capable of handling recursion and non-local references
using call-by-name semantics#cite(<Knuth1964ManOrBoy>, form: "normal").
...
#code-file("chapters/dawn/knuth-man-boy-test.a68", lang: "pascal")


There are some useful applications of call-by-name semantics as well;
it is not #emph[solely] an avenue for confusion.
...
but as Dijkstra points out#cite(<dijkstra_defense_of_algol_60_1961>, form: "normal"),
...
#code-file("chapters/dawn/jensens-device.a68", lang: "pascal")


To simply sum the values of the array #mono-text("V") from 1 to 100, one would call:
#mono-text("Sum(i, 1, 100, V[i])").
The real power of this device is its generality;
if a user were to need to compute $sum_(i=1)^(100) V_(i)times i$,
they could call #mono-text("Sum(i, 1, 100, V[i]*i)").
The first and final arguments to this function are not evaluated
until they are used in the body of the loop in #mono-text("Sum").
...
#code-file("chapters/dawn/swap-error.a68", lang: "pascal")


While calling #mono-text("Swap(i, j)") may work correctly, calling
#mono-text("Swap(i+1, j+1)") or #mono-text("Swap(A[i], A[j])") would not.
