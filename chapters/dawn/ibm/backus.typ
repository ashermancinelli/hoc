...
#cite(<backus_oral_history_2006>, form: "normal").
...
called Speedcoding#cite(<backus_oral_history_2006>, form: "normal"):
...
#figure(
  [#image("../../../resource/ibm-speedcoding-example.png", width: 50%)],
  kind: image,
  caption: [Excerpt from #emph[IBM Speedcoding for the Type 701
			Electronic Data Processing Machine]
		#cite(<IBM_1954_Speedcoding>, form: "normal")]
) <fig:ibm-speedcoding-example>
...
supply. So what the hell?" #cite(<backus_oral_history_2006>, form: "normal") He credits himself
...
#cite(<Backus_1980_Programming_in_America_in_1950s>, form: "normal"). It is unclear whether
...
#cite-title(<Backus_Herrick_1954_Speedcoding>) in which they observe:
...
The question is, can a machine translate a sufficiently rich mathematical
	...
	able to state
	the calculations… for a problem solution in a concise, fairly natural
	mathematical language.
]


The reader should note that it is often incorrectly asserted (at times even by
Backus himself#cite(<Backus_1980_Programming_in_America_in_1950s>, form: "normal")) that this
...
Donald Knuth and Trabb Pardo quote her in #cite-title(<Knuth_TrabbPardo_1976_Early_Development>)
as well.
...
some of which came true and some of which #emph[very much] did not.
...
The document used the term #emph[arithmetic formulas] to describe what
we would today call #emph[assignment statements].

#figure(
  [#image("../../../resource/ibm-7094.jpeg", width: 50%)],
  kind: image,
  caption: [Excerpt from #emph[The IBM 701--7094 II Sequence: A
			Family by Evolution]
		#cite(<Hamming_Feigenbaum_1971_IBM7094>, form: "normal"), illustrating the
...
today call #emph[loops].
Any #emph[formula] could have an associated number with it, allowing that formula to be
...
matrix rotation operations simpler. For example, for a 4x4 matrix #mono-text("b"), the
formula #mono-text("RELABEL(3,1)") would result in #mono-text("b(1,i)") taking the same meaning as #mono-text("b(i,1)")
...
in the FORTRAN system for locating errors in formulas…
...
the problem from its FORTRAN formulation." #cite(<IBM_1954_FORTRAN_Specifications>, form: "normal", supplement: [C. DEBUGGING])
It is remarkable that they #emph[did] consider how to debug FORTRAN programs,
and then #emph[thought better] of providing error messages.
