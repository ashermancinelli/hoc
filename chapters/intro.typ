...
A compiler is a program, not a piece of hardware. A compiler is simply
	...
	contrasts with the interpreter…
	The first completed
	compiler seems to be the A-0 system developed by Dr. Grace Hopper and
	her staff at Remington Rand in 1952…
	#cite(<sammet_programming_languages_history_and_fundamentals_1969>, form: "normal")
]


However, neither definition is sufficiently general.
Two counter examples are TeX and
#mono[METAFONT], which are compilers that transform text into PDF documents and
...
code are consumed; perhaps this was the case with early #mono-text("BASIC")
...
sophisticated compilation techniques. There are even Python #emph[libraries]
that perform just-in-time (JIT) compilation, targeting CPUs, GPUs, and other
specialized hardware
#cite(<jax-compiler>, form: "normal")#cite(<lam-numba>, form: "normal")#cite(<numba_cuda>, form: "normal")#cite(<triton-tillet>, form: "normal").


To capture the full spectrum of compiler technologies, the
definition we will use in this book is intentionally broad:
#quote(block: true)[
#emph[A compiler analyzes, transforms, and produces code, based on source code.]
	 <def:compiler>
]


In the case of TeX and #mono[METAFONT], the source code may not entail a
#emph[program] in the conventional sense; TeX source code describes a
...
music#cite(<aho_oral_history_2022>, form: "normal"). The student's final presentation was to set
...
Another Bell Labs creation was the connection of two
