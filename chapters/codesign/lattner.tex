...
instructions.
His thesis used this example for a simple addressing expression
...
store int 1, int* %p;
;            ^^^^ In modern LLVM IR, this is just 'ptr'
\end{minted}
...
As a brief, opinionated aside: in \parencite{brown_wilson_lattner_aosa_vol1_2011},
...
\begin{quotation}
	...
	implement something similar. The match function and the \texttt{m_} functions allow
	us to perform declarative pattern matching operations on LLVM IR code.
	For example, the \texttt{m_Specific} predicate only matches if the left hand
