...
But there are still lots of interesting open problems left and one of the most
	...
	determined. #cite(<aho_oral_history_2022>, form: "normal")
]
...
#enum(
  [More so than in previous periods, software and
	      hardware must be designed together to give users the best performance.],
  [There is a diverse ecosystem of compiler tools and
...
With the advent of multicore CPUs and #acr-short("simd") instructions in the early 2000s,
this stopped being the case.

For a programmer to get the best possible performance from a CPU that supports #acr-short("simd")
...
represents a step #emph[backwards] in the evolution of compilers and programming languages.
Programming languages were developed #emph[so that] users no longer needed to write (and rewrite)
...
Users have to grapple with not only the #emph[feasability analyses] in the compiler
(which the compiler uses to determine #emph[if] the program has implicit parallelism
that can be exploited with #acr-short("simd") instructions), but also #emph[profitability analyses],
...
#todo[compilers are not debuggable, macros, special flags, leaky abstractions. sometimes
	its good for certain users though.]

#cite(<lattner_golden_age_compiler_design_2021>, form: "normal").
