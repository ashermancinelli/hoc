...
front-ends.
See this example from Doug's TMG manual \parencite{tmg_manual_mcilroy_1972}
...
let V = vec 10 |\textbfit{In BCPL}|
auto V[10];    |\textbfit{In B}|

/* Accessing element i of array V */
V!i            |\textbfit{In BCPL}|
*(V+i)         |\textbfit{In B}|
V[i]           |\textbfit{Also in B}|
\end{minted}
...
Dennis and Brian Kernighan wrote the seminal \citep{the_c_programming_language_1ed_ritchie_kernighan_1989}.
...
Dennis Ritchie as quoted in \parencite{salus_quarter_century_unix_1994}:
...
\subsection{SNOBOL}
\label{sec:software-unix-snobol}
...
\parencite{farber_snobol_string_manip_lang_1964}; the authors also
...
\textit{String Manipulation System for FAP Programs})
...
called \textit{COMIT}, the only language with a syntax similar to that of SNOBOL.
COMIT did not support named variables or arithmetic, which largely motivated
the development of SNOBOL.
\parencite{farber_snobol_string_manip_lang_1964} uses the following program
...
This program removes all the vowels from the string defined by \texttt{TEXT}.
...
discussed in \cref{sec:software-gnu-portable-optimizer}.
...
\subsection{Standard ML at Bell Labs}
...
\parencite{bjarne_stroustrup_design_of_cpp_1995}.
