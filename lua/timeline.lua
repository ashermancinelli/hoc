...
function draw_timeline(config)
    ...
    for _, event in ipairs(events) do
        local year = event[1]
        local desc = event[2]
        local position = event.position  -- Optional custom position for description
        ...
        if type(year) == "table" then
            ...
            -- Use custom position if provided, otherwise use midpoint
            local desc_ypos = position and year_to_y(position) or ypos_mid
            ...
            -- Draw connecting line if position is custom
            if position then
                tex.sprint(string.format("\\draw[red, dashed, thin] (0,%f) -- (0.3,%f);", ypos_mid, desc_ypos))
            end
            
            -- Label at specified position
            tex.sprint(string.format("\\node[description] at (0.3,%f) {%d--%d: %s};", desc_ypos, year_start, year_end, desc))
        else
            local ypos = year_to_y(year)
            ...
            if position then
                tex.sprint(string.format("\\draw[red, dashed, thin] (0,%f) -- (0.3,%f);", ypos, desc_ypos))
            end
            
            -- Draw description at specified position
            -- print(desc_ypos, year, desc)
            tex.sprint(string.format("\\node[description] at (0.3,%f) {%s};", desc_ypos, desc))
        end
    end
